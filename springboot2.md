...
我们后面整合MQ产品都是基于这个案例进行的；

为了便于下面演示各种各样的消息中间件技术，我们**创建一个购物过程生成订单时，为用户发送短信的案例环境，模拟使用消息中间件实现发送手机短信的过程。**
...
1. 存到消息中间件；
2. 从消息队列里面取出来数据，完成真正的业务逻辑；

​		**业务层实现**

​	我们这里采用集合的方式模拟消息队列的处理，先入先出；
...
​		短信处理表现层接口暂且开发出一个处理消息的入口，但是此业务是对应业务层中设计的模拟接口，实际业务不需要设计此接口，我们只是为了看见这个功能；
...
效果：访问order端口表示有订单，后台完成订单处理，并且把要发生消息的这个动作加到消息队列里面；然后访问msg端口——从消息队列里面取出消息，完成短信动作的实现。
...
spring:
  ...
  #jms:
    #template:
      #default-destination: ccooky  
      #这个是我们消息主题的名字，在这里配置就是说所有消息发这个主题，肯定不行的
    
```

**步骤③**：使用`JmsMessagingTemplate`操作ActiveMQ
...
​	发送消息需要先将消息的类型转换成字符串，然后再发送，所以是`convertAndSend`，定义消息发送的位置，和具体的消息内容，此处使用id作为消息内容。
...
public class MessageListener {
    ...
    @JmsListener(destination = "order.queue.id") //当队列里面有消息的时候就会立即取出消息，并且执行下面方法
    @SendTo("other.queue.id")  // 将下面方法的返回值发送给这个消息主题，没有返回值就不会加入队列
    public void receive(String id){  // 传入的这个参数要和加到消息队列的类型一致哦
        System.out.println("已完成短信发送业务，id："+id);
    }
}
```

​	这样每次，他会自动检测队列，假如有消息，他就会马上取出来消息。

**步骤⑤**：切换消息模型由点对点模型到**发布订阅模型**，修改jms配置即可
...
public class MessageServiceKafkaImpl implements MessageService {
    ...
    public void sendMessage(String id) {
        ...
        kafkaTemplate.send("itheima",id); //第一个参数为主题
    }
}
```
...
@Component
public class MessageListener {
    @KafkaListener(topics = "itheima202")
